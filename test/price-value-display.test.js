...
assert.match(source, /尾西の白飯<\/strong><span>5食/);
assert.match(source, /陸上自衛隊戦闘食糧モデル<\/strong><span>4食/);
assert.match(source, /えいようかん<\/strong><span>5本/);
assert.match(source, /非常用臭わないトイレセット<\/strong><span>15回分/);
assert.match(source, /ラジオ付きライト<\/strong><span>1台/);
assert.match(source, /保管箱・内容ラベル・期限一覧<\/strong><span>1式/);
assert.match(lpCss, /\.pack-details-list/);
assert.match(source, /onishi-white-rice\.jpg/);
